...
void ReadCalcSettings(void)
{
  ...
  if ((f=sys_open(fname, A_ReadOnly+A_BIN, P_READ, &err))==-1)
  {
    fname[0]='0';
    f=sys_open(fname, A_ReadOnly+A_BIN, P_READ, &err);
  }
  if (f!=-1)
  {
    if (sys_lseek(f, 0, S_END, &err, &err)==sizeof(CALC_SETTINGS))
    {
      sys_lseek(f, 0, S_SET, &err, &err);
      sys_read(f, &calc_set, sizeof(CALC_SETTINGS), &err);
    }
    sys_close(f, &err);
  }
  else
...
void WriteCalcSettings(void)
{
  ...
  unsigned int err;
  strcpy(fname, setting_file);
  if ((f=sys_open(fname, A_WriteOnly+A_Create+A_Truncate+A_BIN, P_WRITE, &err))==-1)
  {
    fname[0]='0';
    f=sys_open(fname, A_WriteOnly+A_Create+A_Truncate+A_BIN, P_WRITE, &err);
  }
  if (f!=-1)
  {
    sys_write(f, &calc_set, sizeof(CALC_SETTINGS), &err);
    sys_close(f, &err);
  }    
}
...
static void view_settings_ghook(GUI *data, int cmd)
{
  ...
  if (cmd==0x0D)
  {
    if (i==2)
    {
      ...
      {
        str_2ws(ws, drg_def[j - 1], strlen(drg_def[j - 1]));
      }
      else
      {
...
int CreateSettingsEdit(void)
{
  ...
  memcpy(&(view_settings_hdr.rc), header_rect, sizeof(RECT));
  memcpy(&(view_settings_desc.rc), main_area_rect, sizeof(RECT));
